...
To this extent, we define the class :code:`Cell`, aimed at representing the single cell of the sudoku schema, as follows:
...
Thanks to these annotations the :code:`ASPMapper` class will be able to map :code:`Cell` objects into strings properly recognizable from the ASP solver as logic facts of the form *cell(Row,Column,Value)*.
...
The class contains an :code:`Handler` instance as field, that is initialized when the Activity is created as an :code:`AndroidHandler`.
Required parameters include the Android Context (an Android utility, needed to start an Android Service Component) and the type of :code:`AndroidService` to use, in our case a :code:`DLVAndroidService`.
...
It is firstly created an :code:`InputProgram` object that is filled with :code:`Cell` objects representing the initial schema, which is then provided to the handler;
...
For instance, in our case an inner class :code:`MyCallback` implements the interface :code:`Callback`:
